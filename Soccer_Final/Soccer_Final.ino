...
static uint16_t BRAKE_THR = 1400; // RC value below which braking is applied
...
void loop()
{
    ...
    if (readIbusFrame(ch))
    {
        ...
        if(ch[3] < BRAKE_THR) // CH4 for e-stop
        {
            allMotorsBrake();
        ...
        // No differential mix (i.e. leftSpeed = CH1+CH2, rightSpeed = CH1-CH2)
        // Mix is done in the transmitter for better control.
        // Here, just map RC to speed directly for each motor (CH1=left, CH2=right).

        int leftSpeed = rcToSpeed((int)ch[0]); // CH1
        int rightSpeed = rcToSpeed((int)ch[1]); // CH2
